...
-- Migration: 20260828_create_document_verification.sql
-- ==============================================================================
...
-- 2. Create user_roles table for role-based access control (User vs Verifier vs Admin)
...
-- Index for fast lookup by status, user_id and created_at
...
-- Policy 1: Users can SELECT only their own documents; Verifiers/Admins can SELECT ALL documents
...
-- Policy 3: Verifiers and Admins can UPDATE document status and reviewer_note
...
-- Policy 4: Users can DELETE own documents ONLY while status is still 'pending'
...
-- 6. Storage Bucket: user-documents setup
...
-- Storage RLS: Allow authenticated users to upload to user-documents
...
-- Storage RLS: Allow users to read documents (Public or owned)
