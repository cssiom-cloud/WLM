...
import { confirmNotice, escapeHtml, showStatus, withOverlay } from './ui.js';
...
function unitCodeFromUrl() {
  return new URLSearchParams(window.location.search).get('code') || '';
}

function currentUnit() {
  const code = unitCodeFromUrl();
...
function logoMarkup(unit) {
  const image = unit.logo_url
    ? `<img class="unit-logo-lg" src="${escapeHtml(unit.logo_url)}" alt="${escapeHtml(unit.name)}">`
    : `<div class="unit-logo-lg unit-logo-fallback">${escapeHtml(unit.code)}</div>`;
  if (unit.logo_link) {
...
function renderPage() {
  ...
  root.innerHTML = `
    ...
    ${unit.content ? `<p class="unit-briefing">${escapeHtml(unit.content)}</p>` : `<p class="unit-content is-empty">${escapeHtml(t('units.noContent'))}</p>`}
    ${
    ...
    ${manage ? manageMarkup(unit, members, ranks, pendingApps) : ''}
  `;
  initAos();
}
